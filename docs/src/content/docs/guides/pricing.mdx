...
| **Storage fee** | Media storage fee. Audio files up to **10 MB** are stored for free. Above 10 MB, a storage fee is charged that increases progressively with file size. |
...
<Aside type="note">
The **storage fee** only applies when the uploaded audio file exceeds 10 MB. For files up to 10 MB, the storage fee is 0.
</Aside>
