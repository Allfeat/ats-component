...
1. ### Create the token on your backend
   ...
   <Tabs>
     ...
     <TabItem label="cURL">
       ```bash
       curl -X POST https://ats.api.allfeat.org/v1/sessions \
...
<Aside type="tip">
The `accessCode` returned in the `allfeat:complete` event is the end user's key to interact with their work later (update, access). The widget displays it on the success screen for the user to save. Your service may optionally store it as a backup — see [User Management](/#user-management) for details.
</Aside>
