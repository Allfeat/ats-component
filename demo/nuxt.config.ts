...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    ...
    // ATS base URL for *server-side* calls from the proxy route. Distinct from
    // public.atsUrl (the browser-facing one): inside Docker the server reaches
    // the ATS via host.docker.internal, not localhost. Override via
    // NUXT_ATS_API_URL.
    atsApiUrl: "http://localhost:13002",
